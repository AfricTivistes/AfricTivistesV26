...
const NewsletterForm = ({ variant = "inline", idPrefix = "newsletter" }: NewsletterFormProps) => {
  ...
  // Stacked (sidebar) uses inline captcha checkbox; others use floating
  useAltchaCaptcha(captchaRef, variant !== "stacked");
  ...
  return (
    <div>
      <form
        method="post"
        action={INFOMANIAK_ACTION}
        className="inf-form flex flex-col sm:flex-row gap-3"
      >
        {hiddenFields}
        <input
          id={`${idPrefix}-email`}
